...
public class Testdata {
    ...
    User testUser() {
        ...
        return User.builder().id("1").mobileNumber("+32492920000").email("gerd.goemans@student.kdg.be").userPreferences(userPreferences).firstname("John").lastname("Doe")
    ...
    Notification testNotification() {
        return Notification.builder().id("1").userId(testUser().getId()).title("Test").body("This is a test")
            .usedChannel(NotificationChannel.SMS).type(NotificationType.USER_JOINED).urgency(NotificationUrgency.NORMAL)
